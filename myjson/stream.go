...
//makes a new copy of p that's the same length as p
func dupPath(p []string) []string {
...
func (dec *Decoder) EndpToken() (Token, int, int, []string, error) {
	...
	for {
		...
		case '[':
			...
			dec.tokenStack = append(dec.tokenStack, dec.tokenState)
			dec.tokenState = tokenArrayStart
			return Delim('['), start, dec.BytesRead(), dec.genPath(), nil
		...
		case '"':
			if dec.tokenState == tokenObjectStart || dec.tokenState == tokenObjectKey {
				...
				return x, start, dec.BytesRead(), dec.genPath(), nil
			}
			fallthrough
...
func (dec *Decoder) endpTokenError(c byte) (Token, int, int, []string, error) {
	token, err := dec.tokenError(c)
	return token, 0, 0, []string{}, err
}
